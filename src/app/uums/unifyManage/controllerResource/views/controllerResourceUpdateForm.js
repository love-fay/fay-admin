...
import {showUpdatePageForUnifyController, updateForUnifyController} from '../actions';
import {UPDATE_FOR_UNIFY_CONTROLLER_FETCH, UPDATE_FOR_UNIFY_CONTROLLER_SUCCESS, UPDATE_FOR_UNIFY_CONTROLLER_ERROR} from '../actionTypes';
...
const UpdateForm = ({type, res, form, handleSubmit, hideUpdatePage}) => {
    ...
    const {id, appId, menuId, name, sn, urlMapping, order} = res.data;
    const { getFieldDecorator } = form;

    switch (type){
        case UPDATE_FOR_UNIFY_CONTROLLER_FETCH:
            showMessage= 'none';
            message= '';
            messageType= '';
            loading = true;
            break;
        case UPDATE_FOR_UNIFY_CONTROLLER_SUCCESS:
            showMessage= 'block';
            message= '保存成功';
            messageType= 'success';
            loading = false;
            break;
        case UPDATE_FOR_UNIFY_CONTROLLER_ERROR:
            showMessage= 'block';
            message= '保存失败';
            messageType= 'error';
            loading = false;
    ...
    return (
        <Form layout='vertical' onSubmit={(e) => handleSubmit(e, id, appId, menuId)}>
            <FormItem label="名称">
...
const mapStateToProps = (state) => {
    const {updateType, res} = state.uumsUnifyController;
    return {
        type: updateType,
        res: res,
    }
...
const mapDispatchToProps = (dispatch, ownProps) => {
    ...
    const hideUpdatePage = () => {
        dispatch(showUpdatePageForUnifyController(false));
    };

    const handleSubmit = (e, id, appId, menuId) => {
        e.preventDefault();
        ownProps.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                dispatch(updateForUnifyController(values));
            }
        });
    };

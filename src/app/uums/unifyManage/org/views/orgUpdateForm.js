...
import {showUpdatePageForUnifyOrg, updateForUnifyOrg} from '../actions';
import {UPDATE_FOR_UNIFY_ORG_FETCH, UPDATE_FOR_UNIFY_ORG_SUCCESS, UPDATE_FOR_UNIFY_ORG_ERROR} from '../actionTypes';
...
const UpdateForm = ({type, res, form, handleSubmit, hideUpdatePage}) => {
    ...
    const {id, orgId, name, address, phone, orderNum} = res.data;
    const { getFieldDecorator } = form;

    switch (type){
        case UPDATE_FOR_UNIFY_ORG_FETCH:
            showMessage= 'none';
            message= '';
            messageType= '';
            loading = true;
            break;
        case UPDATE_FOR_UNIFY_ORG_SUCCESS:
            showMessage= 'block';
            message= '保存成功';
            messageType= 'success';
            loading = false;
            break;
        case UPDATE_FOR_UNIFY_ORG_ERROR:
            showMessage= 'block';
            message= '保存失败';
            messageType= 'error';
            loading = false;
...
const mapStateToProps = (state) => {
    const {updateType, res} = state.uumsUnifyOrg;
    return {
        type: updateType,
        res: res,
    }
...
const mapDispatchToProps = (dispatch, ownProps) => {
    ...
    const hideUpdatePage = () => {
        dispatch(showUpdatePageForUnifyOrg(false));
    };
    ...
    const handleSubmit = (e, id, orgId) => {
        ...
        ownProps.form.validateFieldsAndScroll((err, values) => {
            if (!err) {
                ...
                dispatch(updateForUnifyOrg(values));
            }
        });
    };
